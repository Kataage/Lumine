...
import { WelcomeScreenV2 } from "../components/NavigationV3";

describe("WelcomeScreen", () => {
  it("アプリ名と案内を表示する", () => {
    render(<WelcomeScreenV2 onSelectFolder={vi.fn()} />);
  ...
  it("ボタンを押すとフォルダー選択を開始する", async () => {
    ...
    render(<WelcomeScreenV2 onSelectFolder={handler} />);
    await user.click(screen.getByRole("button", { name: "画像フォルダーを追加" }));
    expect(handler).toHaveBeenCalledOnce();
  });

  it("処理中はボタンを無効化する", () => {
    render(<WelcomeScreenV2 onSelectFolder={vi.fn()} busy />);
