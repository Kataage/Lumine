...
export function LibrariesPanel({ scanProgress }: { scanProgress: Record<number, ScanProgress> }) {
  ...
  return (
    <div className="pb-3">
      ...
      {error && (
        <div role="alert" className="mx-3 mb-2 rounded-lg border border-destructive/40 bg-destructive/10 px-2.5 py-2 text-[11px] text-destructive">
          {error}
        </div>
      )}
      ...
      <div className="px-2 space-y-1.5">
        {state.libraries.map((library) => {
          ...
          return (
            <div key={library.id} className={`rounded-xl border overflow-hidden ${selected ? "border-primary/35 bg-primary/10" : "border-border bg-muted/10"}`}>
              <button
                type="button"
              ...
              {selected && (
                <div className="border-t border-border/70 px-2 py-1.5 flex items-center gap-1.5">
                  <button type="button" className="ui-mini-button flex-1" onClick={() => void scanLibrary(library.id)} disabled={!library.isEnabled || !!progress}>
                    再スキャン
                  </button>
                  <button
                    type="button"
                    className={`ui-mini-button flex-1 ${managing ? "bg-accent text-foreground" : ""}`}
                    onClick={() => setManagingId(managing ? null : library.id)}
              ...
              {selected && managing && (
                <div className="border-t border-border/70 bg-background/30 px-2.5 py-2.5 space-y-2">
                  ...
                  <button
                    ...
                    onClick={async () => {
                      try {
                        ...
                        await dialog.notify({
                          title: "ライブラリの状態を変更できませんでした",
                          description: "有効/無効の切り替えに失敗しました。",
                  ...
                  >
                    {library.isEnabled ? "ライブラリを無効化" : "ライブラリを有効化"}
                  </button>
                  <button
                    type="button"
                    className="w-full h-8 rounded-lg border border-destructive/35 bg-destructive/10 text-[10px] font-medium text-destructive hover:bg-destructive hover:text-destructive-foreground"
                    onClick={async () => {
                      ...
                      try {
                        ...
                        setState((current) => ({
                          ...current,
                          libraries,
                          selectedLibraryId: current.selectedLibraryId === library.id ? (libraries[0]?.id ?? null) : current.selectedLibraryId,
                          selectedFolderPath: "",
                          detailOpen: false,
                          detailAsset: null,
                          filterTagIds: [],
                        ...
                        await dialog.notify({
                          title: "ライブラリの登録を解除できませんでした",
                          description: "データベースからライブラリ情報を削除できませんでした。",
                          detail: cause instanceof Error ? cause.message : String(cause),
                          tone: "danger",
                        });
                      }
    ...
    return (
      <div key={folder.path}>
        <div className={`flex items-center rounded-lg ${selected ? "bg-primary/10" : "hover:bg-accent/50"}`} style={{ paddingLeft: depth * 10 }}>
          <button
            type="button"
            className="w-7 h-8 text-[10px] flex-shrink-0"
            ...
            aria-label={nested.length ? (open ? `${name} を閉じる` : `${name} を開く`) : undefined}
          >
            {nested.length ? (open ? "▼" : "▶") : ""}
          </button>
          <button
            type="button"
            className="min-w-0 flex-1 h-8 text-left text-[11px] truncate pr-2"
            title={folder.path}
  ...
  return (
    <div className="pb-3">
      ...
      <div className="px-2">
        <button
          type="button"
          className={`w-full h-9 px-2.5 text-left rounded-lg text-xs ${!state.selectedFolderPath ? "bg-primary/10 font-medium" : "hover:bg-accent/50"}`}
    ...
    <div className="pb-3">
      <PanelTitle
        ...
        action={
          <div className="flex gap-1">
            ...
            <button type="button" className={`ui-mini-button ${manageMode ? "bg-accent text-foreground" : ""}`} onClick={() => setManageMode((value) => !value)} aria-pressed={manageMode}>
              管理
            </button>
          </div>
        }
      ...
      <div className="px-3 space-y-2.5">
        ...
        {state.filterTagIds.length > 0 && (
          <button
            type="button"
            className="w-full min-h-8 rounded-lg border border-primary/20 bg-primary/10 px-2.5 text-[10px] font-medium text-primary hover:bg-primary/15"
            onClick={() => setState((current) => ({ ...current, filterTagIds: [] }))}
          >
            ...
            return (
              <div key={tag.id} className={`min-h-9 rounded-lg flex items-center gap-1 ${active ? "bg-primary/12 ring-1 ring-primary/25" : "hover:bg-accent/50"}`}>
                ...
                {manageMode && (
                  <button
                    type="button"
                    className="h-9 px-2 text-[10px] text-muted-foreground hover:text-destructive flex-shrink-0"
                    onClick={async () => {
                      const approved = await dialog.confirm({
                        title: "タグを削除しますか？",
      ...
      <div className="px-3 space-y-3">
        <section className="rounded-xl border border-border bg-muted/10 p-3 space-y-2.5">
          ...
          <div className="flex flex-wrap gap-1">
            {extensions.map((item) => (
              <span key={item} className="h-6 px-2 rounded-full bg-background/60 border border-border text-[9px] inline-flex items-center">{item}</span>
            ))}
          </div>
              ...
              <div className="flex flex-wrap gap-1">
                {extensions.map((item) => (
                  <button
                    key={item}
                    type="button"
                    className="h-7 px-2 rounded-full bg-muted border border-border text-[10px] inline-flex items-center gap-1 hover:border-destructive/40 hover:text-destructive"
                    onClick={async () => {
        ...
        <section className="rounded-xl border border-border bg-muted/10 p-3 space-y-2">
          ...
          <select
            ...
            onChange={async (event) => {
              ...
              await setSetting("conflictPolicy", JSON.stringify(event.target.value));
            }}
          >
