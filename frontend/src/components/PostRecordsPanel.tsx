  ...
  return (
    <div className="px-3 py-3 space-y-3">
      ...
      {showSettings && <div className="rounded-xl border border-border bg-muted/10 p-3 space-y-4">
        ...
        <section className="space-y-2"><p className="ui-label">投稿先</p><form onSubmit={addTarget} className="flex gap-2"><input aria-label="投稿先名" className="ui-input min-w-0 flex-1" value={targetName} onChange={(event) => setTargetName(event.target.value)} placeholder="Pixiv / X など" /><select aria-label="投稿先の種類" className="ui-input w-24" value={targetKind} onChange={(event) => setTargetKind(event.target.value)}><option value="pixiv">Pixiv</option><option value="twitter">X</option><option value="misskey">Misskey</option><option value="bluesky">Bluesky</option><option value="other">その他</option></select><button type="submit" className="ui-primary-button" disabled={targetBusy}>{targetBusy ? "…" : "追加"}</button></form>
        ...
        <section className="space-y-2 border-t border-border pt-3"><p className="ui-label">アカウント</p><form onSubmit={addAccount} className="space-y-2"><select aria-label="アカウントの投稿先" className="ui-input w-full" value={accountTargetId} onChange={(event) => setAccountTargetId(Number(event.target.value))} disabled={!targets.length}><option value={0}>投稿先を選択</option>{targets.map((target) => <option key={target.id} value={target.id}>{target.name}</option>)}</select><div className="flex gap-2"><input aria-label="アカウント表示名" className="ui-input min-w-0 flex-1" value={accountName} onChange={(event) => setAccountName(event.target.value)} placeholder="表示名" disabled={!targets.length} /><input aria-label="アカウントID" className="ui-input min-w-0 flex-1" value={accountIdentifier} onChange={(event) => setAccountIdentifier(event.target.value)} placeholder="@ID（任意）" disabled={!targets.length} /><button type="submit" className="ui-primary-button" disabled={accountBusy || !targets.length}>{accountBusy ? "…" : "追加"}</button></div></form>
