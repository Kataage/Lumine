...
func TestAIAnalysisRepoSemanticClaimPrefersNewestAssetWithinCapability(t *testing.T) {
	database := openAIAnalysisTestDB(t)
	repo := NewAIAnalysisRepo(database)

	lib, err := NewLibraryRepo(database).Create("Semantic Claim", "/tmp/semantic-claim")
	if err != nil {
		t.Fatal(err)
	}
	assetRepo := NewAssetRepo(database)
	makeAsset := func(name string, modified time.Time) int64 {
		t.Helper()
		id, err := assetRepo.Create(&domain.Asset{
			LibraryID:   lib.ID,
			FolderPath:  "/tmp/semantic-claim",
			FileName:    name,
			FilePath:    "/tmp/semantic-claim/" + name,
			Extension:   ".png",
			FileSize:    1,
			ThumbStatus: domain.ThumbStatusNone,
			StatusLabel: domain.StatusUnsorted,
	...
	oldID := makeAsset("old.png", time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC))
	newID := makeAsset("new.png", time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC))
	midID := makeAsset("mid.png", time.Date(2025, 1, 1, 0, 0, 0, 0, time.UTC))

	for _, id := range []int64{oldID, newID, midID} {
	...
	for _, want := range []int64{newID, midID, oldID} {
		job, err := repo.ClaimNext([]domain.AICapability{domain.AICapabilitySemanticSearch})
		if err != nil {
			t.Fatal(err)
		}
		if job == nil || job.AssetID != want {
			t.Fatalf("claimed semantic asset = %+v, want asset %d", job, want)
		}
	}
}

func TestAIAnalysisRepoSemanticRecencyDoesNotBreakCrossCapabilityFairness(t *testing.T) {
	database := openAIAnalysisTestDB(t)
	repo := NewAIAnalysisRepo(database)

	lib, err := NewLibraryRepo(database).Create("AI Fairness", "/tmp/ai-fairness")
	if err != nil {
		t.Fatal(err)
	}
	assetRepo := NewAssetRepo(database)
	create := func(name string, modified time.Time) int64 {
		...
		id, err := assetRepo.Create(&domain.Asset{
			LibraryID:    lib.ID,
			FolderPath:   "/tmp/ai-fairness",
			FileName:     name,
			FilePath:     "/tmp/ai-fairness/" + name,
			Extension:    ".png",
			FileSize:     1,
			ThumbStatus:  domain.ThumbStatusNone,
			StatusLabel:  domain.StatusUnsorted,
	...
	taggerID := create("tagger.png", time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC))
	semanticOldID := create("semantic-old.png", time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC))
	semanticNewID := create("semantic-new.png", time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC))

	taggerJob, _, err := repo.Enqueue(
		taggerID,
		domain.AICapabilityTagger,
		domain.AIJobSourceAutomatic,
		-100,
	...
	// Cross-capability arbitration remains the historical priority -> job-id
	// contract. Semantic recency only changes which semantic job represents
	// that capability.
	first, err := repo.ClaimNext([]domain.AICapability{
		domain.AICapabilityTagger,
		domain.AICapabilitySemanticSearch,
	})
	if err != nil {
	...
	if first == nil || first.ID != taggerJob.ID {
		t.Fatalf("cross-capability fairness changed: first=%+v want tagger job %d", first, taggerJob.ID)
	}

	second, err := repo.ClaimNext([]domain.AICapability{
		domain.AICapabilityTagger,
		domain.AICapabilitySemanticSearch,
	})
	if err != nil {
	...
	if second == nil || second.AssetID != semanticNewID {
