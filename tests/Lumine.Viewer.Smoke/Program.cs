...
var tempRoot = Path.Combine(Path.GetTempPath(), $"lumine-viewer-smoke-{Guid.NewGuid():N}");
...
try
{
    ...
    Console.WriteLine("Viewer smoke: cursor paging / bitmap bounds / cancellation / 100k virtualization OK");
}
finally
{
...
static async Task VerifyDecodedCacheAsync(string tempRoot, string sourceThumbnail)
{
...
static async Task VerifyRequestCoalescingAndCancellationAsync(string thumbnailPath)
{
    ...
    for (var attempt = 0; attempt < 100 && session.Diagnostics.InFlightThumbnailRequests != 0; attempt++)
    {
        await Task.Delay(5);
    }
...
static async Task VerifyHeadlessVirtualizationAsync(string thumbnailPath)
{
    ...
    await headless.Dispatch(
        async () =>
        {
            ...
            Require(viewer.RealizedRowCount is > 0 and < 64, "100k viewer realized an unbounded row count.");
            Require(
            ...
            Require(viewer.RealizedRowCount < 64, "Fast scroll caused row virtualization to expand with library size.");
            Require(viewer.Diagnostics.AttachedTiles < 512, "Fast scroll caused tile count to expand with library size.");

            viewer.SelectAsset(99_999);
            Require(viewer.SelectedAssetIndex == 99_999, "Viewer selection did not reach final 100k asset.");
...
static async Task ExpectCancellationAsync(Task task)
{
    try
    {
        await task;
